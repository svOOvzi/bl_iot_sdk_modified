//  Based on https://github.com/moononournation/Arduino_GFX/blob/master/src/display/Arduino_ST7789.cpp
/*
 * start rewrite from:
...
static void Arduino_TFT_begin(int32_t speed)
{
  ...
  {
    Arduino_SWSPI_begin(speed, _override_datamode);
  }
  else
  {
...
void Arduino_ST7789_invertDisplay(bool i)
{
  Arduino_SWSPI_sendCommand(_ips ? (i ? ST7789_INVOFF : ST7789_INVON) : (i ? ST7789_INVON : ST7789_INVOFF));
}
...
void Arduino_ST7789_tftInit()
{
  ...
  if (_ips)
  {
    Arduino_SWSPI_sendCommand(ST7789_INVON);
  }
}
